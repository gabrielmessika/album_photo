...
`SUIVI_PROJET.md`. Au moment de cette édition, les contrôles iPad et Apple cités
ci-dessous sont tous ⚪ `NON TESTÉ`; aucun scénario d'acceptation n'est donc
...
| `A-GEOM` | [`GeometryEngineTests.swift`](../../Tests/AlbumPhotoCoreTests/GeometryEngineTests.swift), `GeometryEngineTests` | Les quinze méthodes de la classe : fixtures `1×` et `0,5×`, borne dynamique, transformations, cadre initial, hit-testing, guides, zoom de fenêtre et seuils qualité. |
...
|---|---|---|---|
| `ENV-001` à `ENV-005`, `LOT-001`, `LOT-003`, `DONE-005` | Manifeste Swift conservé, `A-MANIFEST` pour les contrats vérifiables | `IPAD-L1-063`, registre entier `IPAD-L1-063` à `IPAD-L1-108` | L'environnement et le statut ne sont prouvés qu'après exécution sur le commit candidat. |
...
| Transactions et stockage : `LOC-001` à `LOC-020`, `LOC-022` à `LOC-031` | `A-STORE`, `A-RAW`, `A-REUSE`, `A-CONC`, `A-CAP`, `A-CLIP` | `IPAD-L1-068`, `IPAD-L1-079`, `IPAD-L1-093`, `IPAD-L1-094`, `IPAD-L1-101` à `IPAD-L1-103`, `IPAD-L1-107`, `IPAD-L1-108`, `APPLE-L1-004`, `APPLE-L1-005` | Le Core distingue désormais l’échec avant publication du nettoyage post-publication et vérifie les blobs en flux ; `LOC-006`, `LOC-019`, `LOC-020` et la purge physique exigent encore les injections Apple différées. `LOC-021` concerne l'export Lot 3. |
...
| Registre et discipline de test : `TST-001`, `TST-002`, `TST-005` à `TST-007`, `TST-011`, `TST-012`, `TST-016` | Cette matrice, suites `Tests/AlbumPhotoCoreTests`, `A-MANIFEST` | Corpus et `IPAD-L1-063` à `IPAD-L1-108`; `APPLE-L1-004` | La présence du lien satisfait la traçabilité structurelle, pas le résultat du test. |
| Procédures et qualification Apple : `TST-003`, `TST-004`, `TST-008` à `TST-010`, `TST-013` à `TST-015` | Les fiches détaillées satisfont la structure de `TST-004`; l'exécution Apple n'est pas réalisable sous Linux | `IPAD-L1-063` à `IPAD-L1-108`, `APPLE-L1-001` à `APPLE-L1-007`, `APPLE-L1-009` | Différé et explicitement non réussi. |

## Matrice des exigences fonctionnelles du lot 1

| Exigences | Automatisation | Contrôles manuels/différés | Qualification actuelle |
|---|---|---|---|
| Navigation applicative `APP-001`, `APP-002`, `APP-005`, `APP-006`, `APP-008` à `APP-011` | `A-CONC`, `A-STORE` | `IPAD-L1-063`, `IPAD-L1-065`, `IPAD-L1-069`, `IPAD-L1-093`, `IPAD-L1-094`, `IPAD-L1-108`, `APPLE-L1-003` | En attente des contrôles Apple. `APP-003`/`APP-004` sont Lot 3 et `APP-007` Lot 4. |
| Bibliothèque et création `ALB-001` à `ALB-004`, `ALB-006`, `ALB-009`, `ALB-011` à `ALB-016`, `ALB-021` | `A-ALB`, `A-CONC` | `IPAD-L1-064` à `IPAD-L1-066`, `IPAD-L1-074` | `ALB-005`, la partie Importer de `ALB-009` et Exporter de `ALB-007` sont Lot 3. |
...
| Corbeille `ALB-008`, `ALB-017` à `ALB-020`, `ALB-023`, `ALB-024` | `A-ALB`, `A-STORE` | `IPAD-L1-067`, `IPAD-L1-068`, `APPLE-L1-004` | Suppression physique sûre et injections exhaustives non encore qualifiées. `ALB-022` est Lot 4. |
| Pages `PAG-001` à `PAG-015` | `A-PAGE`, `A-CAP` | `IPAD-L1-070` à `IPAD-L1-072`, `IPAD-L1-103` | Glisser-déposer, focus, alertes et performances restent manuels. |
...
| Photothèque interne `PHO-001` à `PHO-013`, `PHO-015` à `PHO-018`, `DAT-036`, `DAT-043` | `A-PHOTO`, `A-REUSE`, `A-DOM`, `A-RAW`, `A-STORE` | `IPAD-L1-075`, `IPAD-L1-077` à `IPAD-L1-081`, `APPLE-L1-005` | Tri, sélecteurs, glisser-déposer et corpus réels attendent Apple. `PHO-014`, couplé à Auto, reste Lot 2. |
...
| Canevas/éléments `CAN-001` à `CAN-009`, `ELM-001` à `ELM-014` applicables aux cadres photo | `A-GEOM`, `A-CLIP`, `A-DOM` | `IPAD-L1-083` à `IPAD-L1-086`, `IPAD-L1-095`, `IPAD-L1-099`, `IPAD-L1-100`, `APPLE-L1-003`, `APPLE-L1-007` | Sous-périmètre photo seulement ; texte/sticker attendent Lot 2. |
| Zoom de fenêtre `ZOM-001` à `ZOM-008` | `A-GEOM` | `IPAD-L1-090` à `IPAD-L1-092`, `APPLE-L1-003` | Persistance de session et arbitrage des gestes attendent l'iPad. |
...
| Annuler/Rétablir `UND-001` à `UND-012` | `A-ALB`, `A-PAGE`, `A-PHOTO`, `A-CLIP`, `A-CONC`, `A-TEXT` pour le prototype texte | `IPAD-L1-066`, `IPAD-L1-070`, `IPAD-L1-071`, `IPAD-L1-075`, `IPAD-L1-079`, `IPAD-L1-083`, `IPAD-L1-089`, `IPAD-L1-100`, `IPAD-L1-108` | Types publics Lot 1 seulement. |
| Sauvegarde `SAV-001` à `SAV-004` | Journal et clôture : `A-STORE`, `A-CONC` | `IPAD-L1-093`, `APPLE-L1-011` | Réussite manuelle et échec durable injecté non encore prouvés. |
| Presse-papiers `CLP-001` à `CLP-005` | `A-CLIP` | `IPAD-L1-083` | Partiel : cadre photo public ; texte/sticker Lot 2. |
| Qualité `QLT-001` à `QLT-006` | `A-GEOM` | `IPAD-L1-097`, `APPLE-L1-006` | Export de `QLT-006` attend Lot 3. |
| Interface éditeur `EDT-001` à `EDT-020` applicable aux panneaux Photos/Fonds | État métier indirect : `A-PHOTO`, `A-PAGE`, `A-GEOM` | `IPAD-L1-098`, `IPAD-L1-100`, `IPAD-L1-104`, `IPAD-L1-106`, `APPLE-L1-003` | Partiel : panneaux et commandes Lots 2/3 exclus ; preuve principalement manuelle. |
...
| Capacité/performance Lot 1 : `PAG-012`, `LOC-017`, `LOC-018`, `PERF-008`, `PERF-009`, `PERF-011`, `PERF-015`, `PERF-017` | `A-CAP`, hachage/vérification en flux `A-STORE`/`A-REUSE`, plus tests de déduplication `A-STORE`/`A-RAW` | `IPAD-L1-103`, `IPAD-L1-107`, `APPLE-L1-005`, `APPLE-L1-006` | La mémoire de copie/hachage est bornée, mais le décodage ImageIO et les seuils instrumentés `PERF-001` à `PERF-017` ne sont pas qualifiés sous WSL. |
...
- Aucun résultat manuel n'est encore attaché au candidat : les 46 fiches
  `IPAD-L1-063` à `IPAD-L1-108` ne sont que des procédures.
