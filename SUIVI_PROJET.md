...
| Phase courante | Lot 2 — candidat zones de texte `d882183…` figé ; `IPAD-L2-020` à exécuter |
...
| Candidat zones de texte Lot 2 | `d882183d31de7ed6078c70f9e79a80d6ba994dd6` : création depuis la page et le menu Ajouter, éditeur riche SwiftUI iOS 26, modèles avec texte, rendu partagé, hauteur automatique et débordement ; 146 tests Core et parse AppModule réussis ; `IPAD-L2-020` à exécuter |
...
| Validation indispensable restante | Exécuter `IPAD-L2-020` sur `d882183…` ; couvrir `EDT-004` et la navigation compacte avec `APPLE-L2-001` sur iPhone/Xcode ; compléter justifié, frappe groupée, stickers, cadres et presse-papiers commun |
...
| Lot 2 — Parité de composition | 🟡 | Le dialogue interne, Remplir l’album, l’action compacte et le cadrage couvrant sont validés jusqu’à `IPAD-L2-019`. Le premier incrément texte `d882183…` est figé avec 146 tests Core et parse SwiftUI | Exécuter `IPAD-L2-020`, couvrir la largeur compacte, puis compléter texte, stickers, cadres et presse-papiers commun avant `ACPT-123`, `125`, `126`, `128`, `130` |
...
| Zones de texte | 🟡 | Ajout dans la page et le menu, éditeur riche natif, quatre polices système, tailles 8–96, gras/italique/couleur, paragraphes gauche/centré/droit, interligne, opacité, limite 1 000, hauteur automatique, alerte de débordement, persistance atomique et rendu commun ; candidat `d882183…`, 146 tests Core (`TBX-001…025`, `TXA-001…005`) | Exécuter `IPAD-L2-020` ; justifié, délai 750 ms, export et finition du collage pris en charge restent ouverts |
...
| `IPAD-L2-020`, zones de texte sur `d882183…` | ⚪ Non testé sur Apple | Compiler puis vérifier création/annulation, sélection, styles, limite, modèles texte, profondeur, hauteur, débordement, persistance, orientations et VoiceOver |
...
1. Transférer `d882183d31de7ed6078c70f9e79a80d6ba994dd6` et exécuter
   `IPAD-L2-020` sur l’iPad déclaré, en commençant par sa compilation Apple.
