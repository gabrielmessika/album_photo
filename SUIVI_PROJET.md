...
| Phase courante | Lot 2 — candidat Info et correctifs texte figé dans `60930587da16707dbb57eada9881f6fefcedd51e` ; `IPAD-L2-036…038` prêts à qualifier sur Apple |
...
| Candidat Info et correctifs texte à tester | `60930587da16707dbb57eada9881f6fefcedd51e` : version/build/commit exact dans Info et correctifs texte inchangés ; `IPAD-L2-036…038` à ⚪ `NON TESTÉ` |
...
| Validation indispensable restante | Exécuter `IPAD-L2-036`, puis `037`, puis `038` sur l’archive exacte de `6093058…` ; couvrir `EDT-004` avec `APPLE-L2-001` sur iPhone/Xcode |
| État global | 🟡 **Lot 1 viable sur l'iPad 8 ; motifs, Arrondie/Italique et états actifs sont validés globalement sur `3102cda…`, tandis que le candidat combiné Info `6093058…` reste à qualifier sur Apple et la compacité des boutons de format à reprendre sous `TBX-027`** |
...
| Compacité des boutons de format | Retour après `IPAD-L2-035` : l'ajout de l'état courant dans les libellés principaux rend les boutons trop larges. Le prochain développement conserve un intitulé court et stable ; la valeur active reste visible dans le menu, dans un indicateur compact ou dans la valeur accessible (`TBX-027`, en cohérence avec `TBX-026`). |
...
| Lot 2 — Parité de composition | 🟡 | Le dialogue interne, Remplir l'album, l'action compacte et le cadrage couvrant sont validés jusqu'à `IPAD-L2-019`. La campagne Texte-B sur `cb77862…` confirme compilation/descendantes, panneaux (`031`) et sélection/palette/portées (`032`), puis `IPAD-L2-034…035` qualifient globalement motifs, Arrondie/Italique, états actifs, persistance et VoiceOver sur `3102cda…`. `6093058…` réunit Info et ce correctif texte | Exécuter `IPAD-L2-036…038` sur iPad ; reprendre la compacité `TBX-027`, couvrir la largeur compacte et compléter stickers, cadres et presse-papiers commun avant `ACPT-123`, `125`, `126`, `128`, `130` |
...
l'état courant dans le menu, un indicateur compact et l'accessibilité (`TBX-027`).
...
| Informations de l’application | 🟡 | Bouton `Info` dans la Bibliothèque, version/build lus depuis le bundle, commit complet sélectionnable et accessible ; l’archive `6093058…` a été vérifiée avec son propre hash et Xcode peut surcharger `AlbumGitCommit` (`APP-001`, `APP-012`, `ACC-001`, `ACC-002`) | Exécuter `IPAD-L2-036` : affichage, copie, fermeture, adaptation et VoiceOver sur iPad |
| Texte Photoweb | 🟡 | Domaine pur et adaptateur SwiftUI iOS 26 : conversion 300/72, limite, géométrie, débordement, modèle, profondeur, persistance et accessibilité partielle. `IPAD-L2-031…032` confirment panneaux, sélection, palette et portées sur `cb77862…`; `034…035` qualifient globalement motifs, Arrondie/Italique et états actifs sur `3102cda…` (`TXA-001…005`, `TBX-001…026`) | Prochain développement : réduire la largeur des boutons selon `TBX-027`; justifié `TBX-011`, frappe groupée `TBX-022` et export `TBX-021` restent ouverts |
...
| Traçabilité | 🟡 | Matrices Lot 0/1 et Lot 2, méthodes automatisées, 116 contrôles iPad, 13 validations Apple différées et 24 scénarios `ACPT` ; `034…035` sont 🟢 sur `3102cda…` et `036…038` restent ⚪ sur `6093058…` | Exécuter les trois nouvelles fiches et conserver `008` bloqué jusqu'à une largeur compacte réelle |
...
| Panneaux de création | 🟡 | Les marges, la largeur droite à 324 points et les replis de `024a60b…` sont validés par `IPAD-L2-010`. Le rail régulier de `cb77862…` ordonne Photos, Texte, séparation sans titre, Mise en page, Fonds et réussit `IPAD-L2-031`; Stickers s’insérera avant la séparation (`EDT-001`) | Conserver la preuve iPad ; largeur compacte iPhone/Xcode différée |
...
| Zones de texte | 🟡 | Éditeur riche, styles, limite 1 000, hauteur, débordement, persistance et rendu commun. `030.1…3` et `032` valident compilation, descendantes, sélection, palette et portées sur `cb77862…`; `034…035` qualifient globalement l'arrière-plan strict, Arrondie/Italique et les états actifs sur `3102cda…`. `6093058…` conserve ce correctif (`TBX-001…026`, `TXA-001…005`) | Qualifier le nouveau package par `037…038`; reprendre ensuite les boutons trop larges selon `TBX-027`; justifié, délai 750 ms, export et finition du collage restent ouverts |
...
Italique. Le correctif non figé retire le motif de la pile de mise en page au
profit d’un arrière-plan strict, applique à Arrondie une largeur et une
inclinaison synthétiques publiques, et matérialise chaque choix actif selon
`TBX-026` dans la fenêtre et l’inspecteur.
...
| `RSK-3.0-032` | Levé sur `3102cda…`, `IPAD-L2-037` à tester | Avec chacun des trois motifs intégrés, la couche de fond passait devant le texte et les outils de l'éditeur, alors qu'une couleur unie fonctionnait. | `IPAD-L2-034` réussit globalement les trois motifs sur l'iPad déclaré. Cette preuve reste liée à `3102cda…`; rejouer les motifs sous `037` sur le candidat Info. |
...
| `RSK-3.0-034` | Levé sur `3102cda…`, `IPAD-L2-038` à tester | La différence Système/Arrondie était trop faible et Italique ne produisait aucun changement perceptible avec Arrondie, contrairement aux trois autres polices (`TBX-010`, `TBX-013`). | `IPAD-L2-035` qualifie globalement le profil arrondi élargi et incliné. Cette preuve reste liée à `3102cda…`; vérifier sa conservation sous `038`. |
...
| `RSK-3.0-036` | Levé sur `3102cda…`, `IPAD-L2-038` à tester | Les menus et palettes de format ne matérialisaient pas systématiquement leur valeur active ; une teinte seule restait insuffisante pour comprendre Police, Couleur, Taille et les autres choix. | `IPAD-L2-035` qualifie globalement les huit états visibles et VoiceOver selon `TBX-026`. Cette preuve reste liée à `3102cda…`; `038` contrôle le candidat Info. |
| `RSK-3.0-037` | Moyen, `IPAD-L2-036` à tester | Swift Playgrounds ne fournit pas spontanément le hash Git au runtime ; une constante manuelle deviendrait fausse dès le commit suivant. | L'archive de `6093058…` remplace correctement `$Format:%H$` par son propre hash de 40 caractères. Télécharger cette archive exacte, pas le HEAD futur ; `036` contrôle l'affichage sur iPad et le repli reste `Non estampillé` hors archive. |
| `RSK-3.0-038` | Faible, reprise ergonomique à développer | La concaténation de la valeur courante dans les libellés de Police, Couleur, Taille et autres formats élargit fortement les boutons et réduit l'espace utile. | Implémenter `TBX-027` lors du prochain développement : libellé principal court, état courant conservé dans le menu, un indicateur compact et la valeur accessible ; ajouter une régression Apple sans invalider `IPAD-L2-035`. |

## Prochaines actions

1. Télécharger l’archive exacte de `6093058…`, puis exécuter `IPAD-L2-036`
   pour qualifier Info, la version `0.1.0 (1)` et le hash complet.
2. Exécuter ensuite `IPAD-L2-037`, puis `IPAD-L2-038` sur le même package et
   le même album Texte-A ; ne pas retargeter `034…035`.
3. Lors du prochain développement, réduire la largeur des boutons d'actions
   sur le texte selon `TBX-027`, sans perdre l'état courant exigé par `TBX-026`.
4. Compléter ensuite `TBX-007`, `TBX-011`, `TBX-021` et `TBX-022` sans moteur
   parallèle ni faux contrôle d'interface.
5. Reprendre séparément `APPLE-L2-001` sur un iPhone ou un environnement Xcode
   réellement compact ; ne pas demander le menu Plus au plein écran iPad.
6. Enchaîner ensuite catalogue de stickers/cadres et
...
8. Organiser en parallèle différé les campagnes iPhone, Xcode/macOS,
