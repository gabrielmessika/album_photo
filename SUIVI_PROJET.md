...
| Lot 2 — Parité de composition | 🟡 | Le dialogue interne, Remplir l’album, l’action compacte et le cadrage couvrant sont validés jusqu’à `IPAD-L2-019`. La campagne Texte-B sur `cb77862…` confirme compilation/descendantes, panneaux (`031`), sélection/palette/portées (`032`) et persistance/accessibilité, mais échoue `030` sur les motifs et `033` sur Arrondie/Italique. `3102cda…` borne le fond par construction, renforce Arrondie et ajoute les états actifs `TBX-026` | Exécuter `IPAD-L2-034…035` sur iPad ; couvrir la largeur compacte et compléter stickers, cadres et presse-papiers commun avant `ACPT-123`, `125`, `126`, `128`, `130` |
...
| Traçabilité | 🟡 | Matrices Lot 0/1 et Lot 2, méthodes automatisées, 113 contrôles iPad, 13 validations Apple différées et 24 scénarios `ACPT` ; `031…032` sont 🟢, `030`/`033` 🔴 et `034…035` ⚪ avec candidat exact | Exécuter les deux nouvelles fiches et conserver `008` bloqué jusqu’à une largeur compacte réelle |
...
| Zones de texte | 🟡 | Éditeur riche, styles, limite 1 000, hauteur, débordement, persistance et rendu commun. `030.1…3` et `032` valident compilation, descendantes, sélection, palette et portées sur `cb77862…`; `030.4` et `033.1…2` échouent. `3102cda…` attache le fond au `TextEditor`, renforce Arrondie/Italique et coche chaque valeur active dans l’éditeur et l’inspecteur (`TBX-001…026`, `TXA-001…005`) | Qualifier motifs par `034`, puis Arrondie/Italique et `TBX-026` par `035`; justifié, délai 750 ms, export et finition du collage restent ouverts |
...
| `RSK-3.0-032` | Échec confirmé, `IPAD-L2-034` à tester | Avec chacun des trois motifs intégrés, la couche de fond passe encore devant le texte et les outils de l’éditeur, alors qu’une couleur unie fonctionne. | `IPAD-L2-030.4` invalide le seul `zIndex`; `3102cda…` attache désormais le motif avec `.background` au cadre exact du `TextEditor` et rogne ce cadre. Rejouer les trois motifs sous `034` (`CAN-003`, `BG-007`, `TBX-025`). |
