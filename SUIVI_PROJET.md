...
| Lot 2 — Parité de composition | 🟡 | Le dialogue interne, Remplir l’album, l’action compacte et le cadrage couvrant sont validés jusqu’à `IPAD-L2-019`. `7bc495e…` compile mais échoue sous `IPAD-L2-022`; la conversion typographique corrigée passe 147 tests, les contrats et le parse SwiftUI | Exécuter d’abord `IPAD-L2-023`, puis `024…029` une fiche à la fois, couvrir la largeur compacte et compléter stickers, cadres et presse-papiers commun avant `ACPT-123`, `125`, `126`, `128`, `130` |
...
sans migration ; `IPAD-L2-023…029` reprennent la qualification par petites
fiches en conservant le même album.
...
| Texte Photoweb | 🟡 | Domaine pur et adaptateur SwiftUI iOS 26 : styles, conversion typographique, limite, hauteur, débordement, rendu commun, commandes et modèles texte. Après l’échec de visibilité `022`, l’éditeur, le canevas et les mesures utilisent désormais la même conversion 300/72 (`TXA-001…005`, `TBX-001…025`, `TPL-012`, `TPL-013`, `TPL-017`) | Qualifier d’abord `IPAD-L2-023`, puis `024…029` ; justifié `TBX-011`, frappe groupée `TBX-022` et export `TBX-021` restent ouverts |
| Modèles, dé et automatisme | 🟡 | Les 32 modèles canoniques sont embarqués byte à byte ; résolution active/inactive, bijection photo de `DAT-042`, ordre UUID/lecture, commandes atomiques, dé de session et recomposition Auto sont testés puis exposés. Remplir l’album ajoute son plan pur et sa commande transactionnelle (`AUT-009…011`) ; les variantes texte sont maintenant actives | Qualifier les variantes texte avec `IPAD-L2-028` après les fiches courtes précédentes |
...
| Traçabilité | 🟡 | Matrices Lot 0/1 et Lot 2, méthodes automatisées, 111 contrôles iPad, 13 validations Apple différées et 24 scénarios `ACPT` ; `020…022` sont 🔴 et `023…029` sont préparés à ⚪ avec un jeu partagé | Exécuter d’abord `IPAD-L2-023` et conserver `008` bloqué jusqu’à une largeur compacte réelle |
...
| Zones de texte | 🟡 | Éditeur riche, styles, limite 1 000, hauteur, débordement, persistance et rendu commun. `7bc495e…` corrige le panneau/fond/palette mais `022` révèle la saisie minuscule ; la conversion 300/72 commune corrige le rendu et la mesure sans migrer l’album (`TBX-001…025`, `TXA-001…005`) | Exécuter `IPAD-L2-023`, puis `024…029` dans l’ordre ; justifié, délai 750 ms, export et finition du collage restent ouverts |
...
projection typographique trop petite ; les fiches `023…029` reprennent chaque
fonction séparément avec le même jeu Texte-A.
...
| `RSK-3.0-017` | Levé dans le correctif, à confirmer sur iPad | Les variantes de modèles avec texte étaient visibles mais désactivées afin de ne pas créer une zone vide impossible à saisir. | L’éditeur et `TPL-012` sont intégrés dans le même incrément ; vérifier création, saisie et absence en prévisualisation sous `IPAD-L2-028`. |
...
| `RSK-3.0-026` | Moyen, matérialisé puis corrigé sur `0f4b16c…` | Les API SwiftUI iOS 26 de texte riche ne peuvent être que parsées sous WSL ; `d882183…` a échoué car deux contraintes écrivaient plusieurs clés à travers un proxy borné à leur seule `AttributeKey`. | Les quatre contraintes séparées compilent sur Apple sous `IPAD-L2-021`; la nouvelle conversion doit repasser le contrôle de compilation sous `IPAD-L2-023`. |
...
| `RSK-3.0-028` | Moyen | Le premier incrément valide une session d’édition comme une commande, mais ne publie pas encore chaque séquence après 750 ms ; la conservation fine des styles provenant d’un collage externe reste aussi à compléter. | Garder `TBX-007` et `TBX-022` partiels pendant `IPAD-L2-027`, puis ajouter des tests Apple ciblés dans l’incrément suivant. |
| `RSK-3.0-029` | Élevé, matérialisé deux fois | `IPAD-L2-021` révèle le fond, la palette, l’échelle et l’opacité ; `022` montre ensuite que projeter `N / 3000` directement sur la hauteur affichée rend la saisie pratiquement invisible. | Le rendu et la mesure appliquent maintenant la conversion 300 unités/72 points sans migrer les valeurs persistées ; valider d’abord la frappe sous `IPAD-L2-023`, puis taille/opacité sous `025`. |
| `RSK-3.0-030` | Levé dans la méthode, à confirmer en pratique | Une fiche texte de seize étapes imposait un jeu initial lourd et un défaut précoce empêchait d’attribuer un verdict utile aux fonctions suivantes. | `AGENTS.md` impose désormais des fiches courtes à verdict indépendant et un jeu partagé réutilisé ; `IPAD-L2-023…029` appliquent cette règle séquentiellement. |
