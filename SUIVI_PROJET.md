...
| Second correctif texte à tester | `3102cda0e6b2c483576585ec2f97fc947f87c96f` : arrière-plan strict des motifs, profil Arrondie incliné et états actifs `TBX-026` ; `IPAD-L2-034…035` à ⚪ `NON TESTÉ` |
...
| État global | 🟡 **Lot 1 viable sur l’iPad 8 ; Info et les correctifs texte sont figés et validés sous WSL, mais version/commit, motifs, Arrondie/Italique et états actifs restent à qualifier sur Apple** |
...
| Lot 2 — Parité de composition | 🟡 | Le dialogue interne, Remplir l’album, l’action compacte et le cadrage couvrant sont validés jusqu’à `IPAD-L2-019`. La campagne Texte-B sur `cb77862…` confirme compilation/descendantes, panneaux (`031`), sélection/palette/portées (`032`) et persistance/accessibilité, mais échoue `030` sur les motifs et `033` sur Arrondie/Italique. `6093058…` réunit Info et le second correctif texte | Exécuter `IPAD-L2-036…038` sur iPad ; couvrir la largeur compacte et compléter stickers, cadres et presse-papiers commun avant `ACPT-123`, `125`, `126`, `128`, `130` |
...
par `TBX-026`. Il est figé dans `3102cda…`; `IPAD-L2-034…035` sont prêts et
restent sans preuve Apple.
...
| Traçabilité | 🟡 | Matrices Lot 0/1 et Lot 2, méthodes automatisées, 116 contrôles iPad, 13 validations Apple différées et 24 scénarios `ACPT` ; `034…035` restent ⚪ sur l’ancien candidat et `036…038` sont ⚪ sur `6093058…` | Exécuter les trois nouvelles fiches et conserver `008` bloqué jusqu’à une largeur compacte réelle |
...
| Zones de texte | 🟡 | Éditeur riche, styles, limite 1 000, hauteur, débordement, persistance et rendu commun. `030.1…3` et `032` valident compilation, descendantes, sélection, palette et portées sur `cb77862…`; `030.4` et `033.1…2` échouent. `6093058…` conserve l’arrière-plan strict, Arrondie/Italique et les états actifs du second correctif (`TBX-001…026`, `TXA-001…005`) | Qualifier motifs par `037`, puis Arrondie/Italique et `TBX-026` par `038`; justifié, délai 750 ms, export et finition du collage restent ouverts |
...
| `RSK-3.0-032` | Échec confirmé, `IPAD-L2-037` à tester | Avec chacun des trois motifs intégrés, la couche de fond passe encore devant le texte et les outils de l’éditeur, alors qu’une couleur unie fonctionne. | `IPAD-L2-030.4` invalide le seul `zIndex`; `6093058…` conserve le motif en `.background` au cadre exact du `TextEditor`. `034` reste lié à l’ancien candidat ; rejouer les trois motifs sous `037`. |
...
| `RSK-3.0-034` | Échec confirmé, `IPAD-L2-038` à tester | La différence Système/Arrondie reste trop faible et Italique ne produit aucun changement perceptible avec Arrondie, contrairement aux trois autres polices (`TBX-010`, `TBX-013`). | `IPAD-L2-033.1…2` confirme l’écart ; `6093058…` conserve le profil arrondi élargi de 12 % et la matrice d’inclinaison publique de `0,22`. `035` reste lié à l’ancien candidat ; vérifier sous `038`. |
...
| `RSK-3.0-036` | Corrigé en code, `IPAD-L2-038` à tester | Les menus et palettes de format ne matérialisaient pas systématiquement leur valeur active ; une teinte seule restait insuffisante pour comprendre Police, Couleur, Taille et les autres choix. | `TBX-026` impose une coche ou un état visible et accessible. `6093058…` l’applique aux huit formats dans l’éditeur et l’inspecteur ; `038` contrôle les états visuels et VoiceOver. |
