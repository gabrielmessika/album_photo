...
| Barre principale | Gérer les pages — Vue globale | `square.grid.2x2` | déjà active |
...
| `EDT-016` | Sur iPad, Créer, Gérer les pages et Prévisualiser DOIVENT former un sélecteur de mode visible dans cet ordre. Sur iPhone, la présentation PEUT changer mais ces trois commandes NE DOIVENT PAS être enfouies dans Plus. Le libellé accessible de Créer DOIT inclure Vue page et celui de Gérer les pages Vue globale. |
...
| `EDT-020` | Ajouter une page DOIT rester visible près du zoom et de la navigation de page. Si l’espace résiduel ne permet pas d’afficher son libellé complet sans déborder, le bouton PEUT employer successivement le libellé court Ajouter puis son icône seule ; son libellé accessible complet reste Ajouter une page et aucun texte avec points de suspension NE DOIT être affiché. Le groupe local Ajouter une page, zoom et navigation PEUT passer sur deux rangées ordonnées — ajout et zoom, puis navigation — lorsqu’une seule rangée ne tient pas ; aucune commande NE DOIT être coupée ni imposer un débordement horizontal, et chaque rangée conserve une hauteur de contrôle normale. |
...
| `PAG-017` | Toute commande Ajouter une page DOIT demander confirmation avant de modifier l’album. Le dialogue modal DOIT proposer Annuler, Ajouter la page et une case « Ne plus demander ». Il DOIT être centré au-dessus de l’éditeur, qui DOIT être assombri et rendu non interactif, utiliser au plus `400` points de largeur avec au moins `16` points de marge de chaque côté, et conserver une hauteur minimale de `340` points lorsque l’espace disponible le permet. À la taille de texte standard, ses textes DOIVENT être entièrement visibles sans défilement. Annuler NE DOIT créer aucune page ni modifier le réglage. Lorsque la case est cochée et que l’ajout est confirmé, les ajouts suivants NE DOIVENT plus afficher ce dialogue pendant la session d’édition courante de l’album. Ce réglage DOIT rester modifiable dans Gérer les pages, NE DOIT PAS être sérialisé et DOIT revenir à la confirmation active après fermeture puis réouverture de l’album. |
...
| `GLO-003` | Vue globale DOIT afficher toutes les pages comme une grille adaptative de miniatures numérotées, sans permettre la transformation de leurs éléments. La page active DOIT être identifiable par un contour renforcé et un libellé, sans dépendre de la couleur seule. |
...
| ID | Exigence |
|---|---|
| `SHR-001` | Le panneau Cadres et formes DOIT être actif pour un cadre photo sélectionné et distinguer deux groupes : Cadre décoratif et Contour. Cadre décoratif PEUT séparer visuellement les choix Formes des choix Bordures et motifs, mais ils appartiennent à une sélection commune. |
...
| `SHR-003` | Un cadre photo NE DOIT proposer qu’un seul choix décoratif actif parmi Aucun, les cinq formes non rectangulaires et les six bordures ou motifs. Choisir une forme applique son masque et retire tout cadre décoratif ; choisir une bordure ou un motif restaure le masque Rectangle puis applique sa référence ; Aucun restaure Rectangle et `decorativeFrame = nil`. Chaque changement de portée multiple DOIT rester une commande atomique et annulable. |
...
| `SHR-007` | Les portées Sélection, Toutes les photos de la page et Toutes les photos de l’album DOIVENT être proposées pour un choix de cadre décoratif ou un contour. |
...
| `TBX-004` | Une pression sélectionne la zone ; une seconde pression ou un double toucher place le curseur. Valider confirme la saisie et Annuler restaure le contenu et le style à l’ouverture. |
...
| `TBX-014` | Les tailles DOIVENT être persistées relativement à la hauteur logique de page, avec une plage équivalente à 8–96 points sur la page de référence ; l’interface PEUT afficher l’équivalent en points. La conversion de référence utilise `300` unités canoniques par pouce et `72` points typographiques par pouce : sur une page affichée à la hauteur `H`, une taille de `N` points est donc rendue à `N × H / 720`, sans modifier la valeur canonique déjà persistée. La fenêtre de modification et le canevas DOIVENT appliquer cette même conversion afin que le texte saisi reste lisible et conserve la même échelle visuelle au zoom courant. |
...
| `TBX-028` | Dans la fenêtre de modification, la barre de formatage DOIT être placée immédiatement au-dessus de la surface de saisie et reprendre des conventions bureautiques usuelles : valeurs visibles et compactes pour Police et Taille, puis icônes natives reconnaissables pour Gras, Italique, Couleur, Alignement, Interligne et Opacité. Elle NE DOIT PAS être interrompue par des titres de portée Sélection, Paragraphe ou Zone ; ces portées restent déterminées par la commande et décrites à VoiceOver. |
| `TBX-029` | La fenêtre de modification DOIT réserver à droite de la surface une colonne d’actions fixe, distincte de la barre de formatage, contenant Annuler avec une croix et Valider avec une coche mise en avant. Les deux libellés DOIVENT rester visibles, accessibles et disponibles sans défilement en portrait comme en paysage. |
